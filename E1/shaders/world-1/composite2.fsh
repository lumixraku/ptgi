...
vec4  	GetViewSpacePosition(in vec2 coord, float depth) {	//Function that calculates the screen-space position of the objects in the scene using the depth texture and the texture coordinates of the full-screen quad
		  //depth += float(GetMaterialMask(coord, 5)) * 0.38f;
	vec4 fragposition = gbufferProjectionInverse * vec4(coord.s * 2.0f - 1.0f, coord.t * 2.0f - 1.0f, 2.0f * depth - 1.0f, 1.0f);
		 fragposition /= fragposition.w;
...
void NetherHeatRefraction(inout SurfaceStruct surface)
{
	/*
	float rayDepth = 0.02;
	float increment = 5.0;
	...
	vec2 refractAccum = vec2(0.0);

	int count = 0;

	while (rayDepth < rayLimit)
	{
		if (surface.linearDepth < rayDepth)
		{
			break;
		}

		vec4 rayPosition = GetViewSpacePosition(texcoord.st, LinearToExponentialDepth(rayDepth));
		rayPosition = gbufferModelViewInverse * rayPosition;

		vec2 noiseSample = vec2(Get3DNoise(rayPosition.xyz), Get3DNoise(rayPosition.xyz + vec3(0.5))) * 2.0 - 1.0;

		refractAccum += noiseSample;

		rayDepth += increment;
		count++;
	}

	vec2 refractCoord = texcoord.st + refractAccum * 0.001;
*/

	float aspectCorrect = viewHeight / viewWidth;
