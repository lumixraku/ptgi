...
float d(float v,float t)
 ...
 void main()
 {
   ...
   float n=t(texcoord.xy);
   vec2 z=vec2(0.);
   float r=1.+3./(i*200.*gPdrYTep+pehfXVXY);
   r*=1.6*mix(uwtHmhPW,KLKQcJfi,f);
   float C=mix(1000.*CRgczcVK,1000.*LwgeyngM,f);
   C/=i*100.+.5;
   float x=62.,B=2.;
   ...
   for(int s=-1;s<=1;s++)
     {
       for(int c=-1;c<=1;c++)
         {
           vec2 A=(vec2(s,c)+z)/vec2(viewWidth,viewHeight)*(6.5+6.*f)*r,l=texcoord.xy+A.xy;
           ...
           float w=t(l),P=pow(saturate(dot(h,p)),x),R=exp(-(abs(w-n)*B)),I=exp(-(t(D,g)*C)),S=P*R*I;
           L+=D*S;
           a+=S;
           T+=D;
           o++;
