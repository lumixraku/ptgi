...
vec3 TraceVolumeShadow(vec3 worldDir)
{
	...
	for (int i = 0; i < 512; i++)
	{
		...
		vec3 rayVolumePos = HAWDRSEJ(rayPos);

		vec2 lookupCoord = DiuEETJJ(rayVolumePos) + vec2(0.0 / vec2(2048.0, 2048.0));
...
{
	vec2 coord = OhwBTsdT(volumeCoord);
...
vec3 ReadLPV(vec3 worldPos, vec3 worldNormal)
{
	...
	vec3 volumePos = SrTVycDN(worldPos);
...
vec3 TraceVolumeBuffer(vec3 worldDir)
{
	...
	// 	vec3 rayVolumePos = SrTVycDN(rayPos);

	// 	vec2 lookupCoord = DiuEETJJ(rayVolumePos);
	...
	int volumeSize = frzHVXWE();

	origin = HAWDRSEJ(origin) * volumeSize;
	...
	for (int c = 0; c < 60; c++)
	{
		...
		vec2 lookupCoord = DiuEETJJ(volumeCoord);
...
vec3 VisualizeSecondaryBounce(vec3 worldPos, vec3 worldNormal)
{
	worldPos += Fract01(cameraPosition.xyz + 0.5) - 0.5;

	vec3 volumeCoord = SrTVycDN(worldPos + worldNormal * 0.1);
	
	vec3 data = SdwzegmK(OhwBTsdT(volumeCoord)).iGQYSjKr;
...
void main() 
{
	...
	finalComposite.rgb = GetPathtraceGI(texcoord.st, gbuffer.normal, gbuffer.depth, viewPos.xyz) * gbuffer.albedo.rgb * 10.0;

	


	//sunlight
	float sunlightMult = 24.0 * (1.0 - rainStrength);
...
//void CloudPlane(inout vec3 color, vec3 viewDir, vec3 worldVector, float linearDepth, MaterialMask mask, vec3 worldLightVector, vec3 lightVector)
	...
	// finalComposite.rgb = VisualizeSecondaryBounce(worldPos.xyz, worldNormal.xyz) * gbuffer.albedo.rgb;
