...
void main() 
{
	...
	filterWidth *= 0.9 * mix(BlurRadius, BlurRadiusDisocclusion, disocclusion);
	// filterWidth = 0.0;
	...
	float colorRejection = mix(800.0 * ColorBlendRejection, 450 * ColorBlendRejectionDisocclusion, disocclusion);
	// colorRejection *= 1.0 - disocclusion * 0.25;
	...
	temporalBuffer1.rgb = mix(temporalBuffer1.rgb, gi.rgb, vec3(mix(saturate(0.0 * BlurBlendWeight / (prevGILuminance + 0.04)), 1.0, disocclusion)));
