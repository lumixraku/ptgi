...
const int 		shadowMapResolution 	= 4096;	// Shadowmap resolution [1024 2048 4096]
...
vec3 TraceVolumeShadow(vec3 worldDir)
{
	vec3 origin = vec3(0.0);


	//origin += vec3(-19.0, 13.5, -1.0);
	origin += vec3(-1.0, 0.5, -1.0);
	origin += fract(cameraPosition.xyz + 0.5);
	//origin -= 0.5;
	...
	for (int i = 0; i < 512; i++)
	{
		vec3 rayPos = origin + rayDir * (i * 0.25);

		vec3 rayVolumePos = WorldToVolumeShadow(rayPos);

		vec2 lookupCoord = VolumeCoordToTexcoordShadow(rayVolumePos) + vec2(0.0 / vec2(2048.0, 2048.0));

		vec4 raySample = texture2D(shadowcolor, lookupCoord);

		if (raySample.x < 0.49 || raySample.y < 0.49 || raySample.z < 0.49)
		{
			result = raySample.rgb;
			break;
		}
...
SHCoeffs DecodeSHCoeffs(SHCoeffs coeffs)
{
	coeffs.red = coeffs.red 	* 2.0 - 1.0;
	coeffs.green = coeffs.green * 2.0 - 1.0;
	coeffs.blue = coeffs.blue 	* 2.0 - 1.0;
	coeffs.alpha = coeffs.alpha * 2.0 - 1.0;
	...
	coeffs.red = pow(abs(coeffs.red), 		vec4(SH_ENCODE_POWER)) * sign(coeffs.red);
	coeffs.green = pow(abs(coeffs.green), 	vec4(SH_ENCODE_POWER)) * sign(coeffs.green);
	coeffs.blue = pow(abs(coeffs.blue), 	vec4(SH_ENCODE_POWER)) * sign(coeffs.blue);
	coeffs.alpha = pow(abs(coeffs.alpha), 	vec4(SH_ENCODE_POWER)) * sign(coeffs.alpha);

	return coeffs;
}


SHCoeffs SampleCoeffsFromVolume(vec3 volumeCoord)
{
	vec2 coord = VolumeCoordToTexcoord(volumeCoord);

	vec4 sample0 = texture2DLod(gaux1, coord, 0);
	//sample0 = sample0 * 2.0 - 1.0;
	vec4 sample1 = texture2DLod(gaux2, coord, 0);
	//sample1 = sample1 * 2.0 - 1.0;

	SHCoeffs result;

	result.red.rg 	= UnpackTwo16BitFrom32Bit(sample0.r);
	result.red.ba 	= UnpackTwo16BitFrom32Bit(sample1.r);

	result.green.rg = UnpackTwo16BitFrom32Bit(sample0.g);
	result.green.ba = UnpackTwo16BitFrom32Bit(sample1.g);

	result.blue.rg 	= UnpackTwo16BitFrom32Bit(sample0.b);
	result.blue.ba 	= UnpackTwo16BitFrom32Bit(sample1.b);

	result.alpha.rg = UnpackTwo16BitFrom32Bit(sample0.a);
	result.alpha.ba = UnpackTwo16BitFrom32Bit(sample1.a);

	result = DecodeSHCoeffs(result);

	return result;
}

vec3 ReadLPV(vec3 worldPos, vec3 worldNormal)
{
	//worldPos += rand(texcoord.st + sin(frameTimeCounter)).rgb - 0.5;

	worldPos += fract(cameraPosition.xyz + 0.5) - 0.5;

	worldPos += worldNormal * 0.2;

	vec3 volumePos = WorldToVolume(worldPos);

	SHCoeffs lpvCoeffs = SampleCoeffsFromVolume(volumePos);

	vec3 result = FromSH(lpvCoeffs.red, lpvCoeffs.green, lpvCoeffs.blue, worldNormal.xyz) * 5.0;
...
vec3 TraceVolumeBuffer(vec3 worldDir)
{
	vec3 origin = vec3(0.0);
	...
	float blockMaterialID = 0.0;

	// for (int i = 0; i < 512; i++)
	// {
	// 	vec3 rayPos = origin + rayDir * (i * 0.25);

	// 	vec3 rayVolumePos = WorldToVolume(rayPos);

	// 	vec2 lookupCoord = VolumeCoordToTexcoordShadow(rayVolumePos);
	...
	origin = WorldToVolumeShadow(origin) * volumeSize;

	//Ray ray = Ray(origin, worldDir);
	Ray ray;
	ray.origin = origin;
	ray.dir = worldDir;
	...
	for (int c = 0; c < 60; c++)
	{
		...
		//TODO: Implement out of bounds termination

		vec3 volumeCoord = vec3(coord) / float(volumeSize);
		vec2 lookupCoord = VolumeCoordToTexcoordShadow(volumeCoord);
	...
	//result = vec3(blockMaterialID);

	return result;
}

vec3 PseudoBlueNoise(vec2 coord)
{
	...
	vec3 whiteNoise = texture2DLod(noisetex, noiseCoord.st, 0).rgb;

	float spread = 0.45;
	float sub = 0.25;

	vec3 blurNoise = texture2DLod(noisetex, noiseCoord.st + vec2(spread, spread) * vec2(1.0 / 64.0), 0).rgb * 	0.25;
	blurNoise += texture2DLod(noisetex, noiseCoord.st + vec2(spread, -spread) * vec2(1.0 / 64.0), 0).rgb * 		0.25;
	blurNoise += texture2DLod(noisetex, noiseCoord.st + vec2(-spread, spread) * vec2(1.0 / 64.0), 0).rgb * 		0.25;
	blurNoise += texture2DLod(noisetex, noiseCoord.st + vec2(-spread, -spread) * vec2(1.0 / 64.0), 0).rgb * 	0.25;

	vec3 blueNoise = (whiteNoise - blurNoise) + 0.5;

	//blueNoise = pow(blueNoise, vec3(1.2));

	//blueNoise = mod(blueNoise + frameTimeCounter * 1.0, vec3(1.0));

	blueNoise = blueNoise * (2.0) - (127.0 / 255.0);

	//blueNoise = rand(noiseCoord).rgb;
...
vec3 RayTraceShadow(vec3 worldPos, vec3 worldNormal, vec3 worldDir, float mcSkylight)
{
	...
	float seed = (texcoord.x + texcoord.y * 3.4321 + fract(frameCounter * 0.001) * 10.0) * 9.1;
	...
	float mcSkylightWeight = pow(mcSkylight * mcSkylight * (3.0 - 2.0 * mcSkylight), 0.1);
	...
	for (int i = 0; i < numSamples; i++)
	{
		...
		rayDir = worldLightVector;
		// rayDir = normalize(rayDir * 0.1 + worldLightVector);

		vec3 rayOrigin = worldPos + worldNormal * 0.01;
		rayOrigin += fract(cameraPosition.xyz + 0.5);
		...
		vec3 absorption = vec3(1.0);
			...
			if (numStepsCompleted >= 99)
			{
				// lightSum += vec3(0.5, 0.8, 2.0) * absorption * 0.105;
			...
			ray.dir = cosWeightedRandomHemisphereDirection(-sideNormals[side] * sign(ray.dir[side]), seed, i);
	...
	lightSum *= saturate(dot(worldNormal, worldLightVector));
...
{
	return ExpToLinearDepth(texture2DLod(depthtex1, coord * 2.0, 0).x);
}
...
vec3 GetPathtraceGI(vec2 coord, vec3 normal, float depth, vec3 viewPos)
{
	#ifndef HALF_RES_TRACE
	return texture2DLod(SOURCE_TARGET, coord, 0).rgb;
	#else

	#if 0
	return texture2DLod(SOURCE_TARGET, coord * 0.5, 0).rgb;
	#else
	...
	normal = GetNormal(coord * 0.5);
	depth = GetLinearDepth(coord * 0.5);
	#else
	normal = GetNormal(coord);
	depth = GetLinearDepth(coord);
	#endif

	float normalRejectionWeight = 1.0;
	float depthRejectionWeight = 5.0;

	// vec2 dither = rand(coord + sin(frameTimeCounter)).xy - 0.5;
	vec2 dither = BlueNoise(coord.xy).xy - 0.5;
	// dither *= 0.0;
	...
	for (int i = -1; i <= 1; i++)
	{
		...
		{
			#ifdef HALF_RES_TRACE
			vec2 lookupCoord = ((vec2(i, j) + dither.xy) / vec2(viewWidth, viewHeight)) * offset + coord.st * 0.5;
			vec2 lookupCoordDepth = ((vec2(i, j) + dither.xy) / vec2(viewWidth, viewHeight)) * offset * 1.5 + coord.st * 0.5;
			#else
			vec2 lookupCoord = ((vec2(i, j) + dither.xy) / vec2(viewWidth, viewHeight)) * offset + coord.st;
			vec2 lookupCoordDepth = ((vec2(i, j) + dither.xy) / vec2(viewWidth, viewHeight)) * offset * 1.5 + coord.st;
			#endif

			// lookupCoord = (floor(lookupCoord * vec2(viewWidth, viewHeight) * 0.5) + 0.5) / vec2(viewWidth * 0.5, viewHeight * 0.5);

			float depthSample = GetLinearDepth(lookupCoordDepth);
			vec3 normalSample = GetNormal(lookupCoordDepth);
			vec3 colorSample = texture2DLod(SOURCE_TARGET, lookupCoord, 0).rgb;
			...
			weightSum += weight;
			dumbBlur += colorSample;

			c++;
		}
	}

	sum /= weightSum + 0.00001;
...
void main() 
{
	...
	//GI
	vec4 gi = GetGI(gbuffer.albedo.rgb, gbuffer.normal, gbuffer.mcLightmap.g);
	...
	finalComposite += skylight * gbuffer.albedo.rgb * 2.0 * ao;
	...
	{
		finalComposite += gbuffer.albedo.rgb * 8.0;
	}
	...
	// //sunlight
	// float sunlightMult = 12.0 * exp(-contextualFogFactor * 2.5) * (1.0 - rainStrength);
	...
	// //finalComposite += SpecularGGX(worldNormal, -worldDir, worldLightVector, pow(1.0 - gbuffer.smoothness, 1.5), gbuffer.metalness * 0.98 + 0.02) * sunlightMult * shadow;
	...
	//finalComposite.rgb = texture2D(gaux3, texcoord.st * 0.5).rgb * gbuffer.albedo.rgb * 5.0;
	finalComposite.rgb = GetPathtraceGI(texcoord.st, gbuffer.normal, gbuffer.depth, viewPos.xyz) * gbuffer.albedo.rgb * 5.0;
	// finalComposite += RayTraceShadow(worldPos.xyz, worldNormal.xyz, worldDir.xyz, 1.0) * gbuffer.albedo.rgb * 10.0 * colorSunlight;

	// finalComposite = vec3(dot(worldNormal, normalize(vec3(1.9, 1.55, 1.0))) * 0.5 + 0.5);
	...
	if (materialMask.sky > 0.5 || gbuffer.depth > 0.9999999)
	{
		...
		finalComposite += sunDisc * 0000.0 * pow(1.0 - rainStrength, 5.0);
...
//void CloudPlane(inout vec3 color, vec3 viewDir, vec3 worldVector, float linearDepth, MaterialMask mask, vec3 worldLightVector, vec3 lightVector)
	...
	// finalComposite = mix(finalComposite, TraceVolumeShadow(worldDir), vec3(1.0));
	//finalComposite = texture2D(gaux3, texcoord.st).rgb;
	//finalComposite = texture2D(shadowcolor, texcoord.st).rgb;


	//finalComposite.rgb = vec3(gbuffer.smoothness);



	//finalComposite.rgb = ReadLPV(worldPos.xyz, worldNormal.xyz) * gbuffer.albedo.rgb;
	// finalComposite.rgb = PathTraceGI(worldPos.xyz, worldNormal.xyz, worldDir.xyz) * gbuffer.albedo.rgb;



	if (materialMask.glowstone > 0.5)
	{
		finalComposite.rgb += gbuffer.albedo.rgb * 5.0;
	}


	// finalComposite = mix(finalComposite, TraceVolumeBuffer(worldDir), vec3(1.0));

	//finalComposite.rgb = texture2D(gaux1, texcoord.st).rgb;


	// BBRay testRay = MakeRay(vec3(0.0), worldDir);

	// float tmin, tmax;
	// vec3 aabb[2] = vec3[2](vec3(1.0, 1.0, 1.0) - 0.15, vec3(1.0, 1.0, 1.0) + 0.15);
	// intersection_distances_no_if(testRay, aabb, tmin, tmax);

	// if (tmax >= tmin)
	// if (isinf(tmin))
	// {
		// finalComposite.r = 1.00;
	// }


	// finalComposite.r = texture2D(gaux2, texcoord.st).x;

	finalComposite *= 0.001;
	...
	//float test = PackTwo16BitTo32Bit(finalComposite.rg);
	//finalComposite.rg = UnpackTwo16BitFrom32Bit(test);
	...
	// gl_FragData[1] = texture2D(gaux1, texcoord.st);
	// gl_FragData[2] = texture2D(gaux2, texcoord.st);
}
