...
vec4 ComputeFakeSkyReflection(vec3 dir, vec3 normal, MaterialMask mask)
{
	...
	sky = mix(sky, vec3(0.6) * Luminance(colorSkylight), vec3(rainStrength * 0.95));
	vec3 skyNight = AtmosphericScattering(worldDir, -worldSunVector, 1.0) * 0.00025;
	skyNight = mix(skyNight, vec3(0.6) * 0.00025, vec3(rainStrength * 0.95));
...
void main() 
{
	...
	gbuffer.normal = normalize(gbuffer.normal - viewDir.xyz * (1.0 / (saturate(dot(gbuffer.normal, -viewDir.xyz)) + 0.01) ) * 0.0085);
