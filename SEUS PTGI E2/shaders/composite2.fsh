...
void main() 
{
	...
	filterWidth *= 0.3 * mix(BlurRadius, BlurRadiusDisocclusion, disocclusion);
	// filterWidth = 0.0;

	vec3 dumbBlur = vec3(0.0);


	float colorRejection = mix(600.0 * ColorBlendRejection, 300 * ColorBlendRejectionDisocclusion, disocclusion);
	// colorRejection *= 1.0 - disocclusion * 0.5;
	...
	temporalBuffer1.rgb = mix(temporalBuffer1.rgb, gi.rgb, vec3(mix(saturate(0.1 * BlurBlendWeight / (prevGILuminance + 0.02)), 1.0, disocclusion)));
